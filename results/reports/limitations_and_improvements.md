...
### 2.1. Nguy cơ aliasing khi resampling
...
### 2.2. FFT không áp dụng window function
...
### 2.4. Bland-Altman sử dụng population standard deviation
...
### 4.2. Code thừa không được sử dụng
...
### 4.3. Trùng lặp code (code duplication)
...
### 4.4. Giá trị hardcode không qua config
...
### 5.1. Không có phân tích coherence
...
### 5.2. Thiếu confidence interval và uncertainty quantification
...
### 5.3. Không kiểm tra giả định phân phối chuẩn
...
4. **Resampling không có anti-aliasing filter** — Downsample từ 1651 Hz xuống 500 Hz bằng nội suy tuyến tính có thể gây aliasing trong dải 0–250 Hz. Tuy nhiên, phân tích tần số được thực hiện trên tín hiệu gốc nên ít bị ảnh hưởng.

5. **Chưa phân tích coherence function** — Coherence là tiêu chuẩn vàng cho so sánh cảm biến rung theo tần số, nhưng chưa được tích hợp trong phân tích.

6. **Chỉ so sánh một trục đo** — Chưa phân tích đầy đủ 3 trục và vector magnitude.

### Mức độ quan trọng thấp (có thể nêu)

7. **FFT không sử dụng window function** — Gây spectral leakage, nhưng PSD (Welch) có window nên ảnh hưởng hạn chế.

8. **Không có uncertainty quantification** — Các chỉ số được báo cáo không kèm confidence interval.
...
| 3 | Tích hợp coherence analysis | Đánh giá tương quan tuyến tính theo tần số |
| 4 | Phân tích nhiều cửa sổ + bootstrap CI | Tăng độ tin cậy thống kê |
...
| 8 | Unit test với synthetic signals | Xác nhận tính đúng đắn của pipeline |
| 9 | Anti-aliasing filter trước resampling | Loại bỏ nguy cơ aliasing |
| 10 | Thêm normality test cho Bland-Altman | Xác nhận giả định thống kê |
...
- Sử dụng phương pháp Bland-Altman chuẩn (tham chiếu đúng Bland & Altman, 1986)
- Phân tích đa miền: thời gian, tần số, tương quan, sai số
...
- **Phương pháp:** Thiếu coherence analysis, confidence interval, stationarity check
- **Kỹ thuật:** Thiếu unit test, code thừa, hardcoded parameters, aliasing risk

...
- Cua so 100s bat dau `2026-02-28T02:23:30Z` (UTC that)
- Resample 500 Hz
- Alignment: tat (dung timestamp UTC tuyet doi truc tiep)

### Luu y timezone

- **Sensor A**: epoch UTC chuan → `timezone_offset_hours: 0`
- **Sensor B**: epoch theo dong ho UTC+7 → `timezone_offset_hours: -7`
...
- Neu `datetime_utc` parse sai / hai sensor khong overlap:
  - Kiem tra `timezone_offset_hours`: sensor A = 0, sensor B = -7
  - Xac nhan lai `window.start_utc` nam trong khoang ca hai sensor co du lieu (UTC that)
